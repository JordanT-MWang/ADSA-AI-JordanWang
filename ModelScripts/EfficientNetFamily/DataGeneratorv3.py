...
class ADSADataPipeline:
    ...
    def _parse_function(self, path, param, y):
        ...
        # Compute scale factor for padding
        ...
        # Adjust scale factor in parameters
    ...
    def _generator(self):
        # Yield samples one by one (lazy)
        for path, param, y in zip(self.image_paths, self.params, self.outputs):
            yield path, param, y
    ...
    def get_dataset(self):
        ...
        # Shuffle if needed
        if self.shuffle:
            ds = ds.shuffle(buffer_size=1000, seed=self.random_state)
